...
$('.drop-down__menu').on('click', function (e) {
	e.preventDefault();
	$('.drop-down__list').toggleClass('hidden');
	$('.drop_dawn__arrow').toggleClass('rotate');
})
$('.drop-down__menu').on('dblclick', function (e) {
	e.preventDefault();
	$('.drop-down__list').toggleClass('hidden');
	$('.drop_dawn__arrow').toggleClass('rotate');
})
$(document).mouseup(function(e){
	if(!$('.drop-down__menu').is(e.target)&& $('.drop-down__menu').has(e.target).length === 0){
		$('.drop-down__list').addClass('hidden');
		$('.drop_dawn__arrow').removeClass('rotate');
	}
	if(!$('ul.wooder__nav ').is(e.target)&& $('ul.wooder__nav ').has(e.target).length === 0 ){
		$('.wooder__nav ').removeClass('active');
		$('.menu-header__icon').removeClass('active');
	}
	if($('.menu-header__icon').is(e.target)) {
		console.log('r')
		$('.wooder__nav').toggleClass('active');
		$('.menu-header__icon').toggleClass('active');
	}
	var li = $('.drop-down__menu ul').find('li');
	if(li.is(e.target)){
		$('.drop-down__menu span.lang').html($(e.target).html());
	}
})

//</ВИПАДАЮЧЕ МЕНЮ>

//<ДОДАВАННЯ КЛАСУ ACTIVE ПРИ КЛІКУ НА БУРГЕР>
...
$('.gotofurn').click(function(e) {
	gotarg('furniture')
});
$('.gotoqual').click(function(e) {
	gotarg('quality')
});
$('.gotoabout').click(function(e) {
	gotarg('about-us')
});
$('.gotowooder').click(function(e) {
	gotarg('wooder')
});
function gotarg(targ){
	var offset=0;
	$('body,html').animate({scrollTop:$('section.'+ targ).offset().top},800);
	return false;
}
...
function mainblock(){
		var h=$(window).outerHeight();
	$('.full__section').css('min-height',h);
}
	mainblock();
...
$(window).scroll(function(event) {
		...
		w = $(window).outerWidth();
	$('.wooder__bg-left.ibg').css('backgroundPositionY',s+'px');
	$('.wooder__bg-right.ibg').css('backgroundPositionY',s+'px');
	if(w < 2000){
		$('.column-t__r .opasity-big-text').css('left','00%');
		$('.column-b__l .opasity-big-text').css('left','-5%');
	} else {
		$('.column-t__r .opasity-big-text').css('left',(s+58)/8+'%');
		$('.column-b__l .opasity-big-text').css('left',-(s-153)/8+'%');
	}
...
// </GOTO>
// <rise-block>
$('.rise-block-02').removeClass('.absol');
let riseArr = [];
riseArr = $('.rise-block-02');
for(i = 0; i < riseArr.length;i++){
	let widItem = $(riseArr[i]).outerWidth(true),
	heigItem = $(riseArr[i]).outerHeight(true);
	var temp = $(riseArr[i]).eq(0).clone()	
	$('<div class="new"></div>').addClass('new'+i).addClass('relat').width(widItem).height(heigItem).insertBefore($(riseArr[i]));
	$(riseArr[i]).eq(0).remove()
	
	
	$(temp).addClass('absol').addClass('opas-none').appendTo('.new'+i); 
}

 let time = 0
 $(window).scroll(function(event) {
 	$('.rise-block-02').each(function(i, el){
 		let s1=$(window).scrollTop(),
		w = $(window).outerHeight(),
		t = $(el).offset().top,		
		t0 = t-s1-w/.6;
		if(t0 <= 0){
			$(el).css({'opacity': '1', 'top': '0px','transition': 'all 0.7s cubic-bezier(0.68,-0.55,0.265,1.55) '+time+'s'})
		}
		time = time + 0.1
		if(time > .8){
			time = 0
